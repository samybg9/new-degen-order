...
import {
  ...
  getAddToWhitelistTransaction,
  getEnableWhitelistTransaction,
  getDisableWhitelistTransaction,
} from './utils';
import {
  ...
  const promptQuestions: PromptObject[] = [
    {
      type: 'text',
      name: 'addressToAdd',
      message: 'Address to add to the whitelist: ',
      validate: (value) => (!value ? 'Required!' : true),
    } 
  ];

  try {
    const { addressToAdd } = await prompts(
      promptQuestions
    );

    if (!addressToAdd) {
      console.log('You have to provide the address to add to the whitelist!');
      exit(9);
    }
    ...
    const addToWhitelistTx = getAddToWhitelistTransaction(
      smartContract,
      6000000,
      addressToAdd
    );

    await commonTxOperations(addToWhitelistTx, userAccount, signer, provider);
  } catch (e) {
    console.log(e);
  }
};
...
const enableWhitelist = async () => {
  ...
  try {
    ...
    const enableWhitelistTx = getEnableWhitelistTransaction(
      smartContract,
      5000000
    );

    await commonTxOperations(enableWhitelistTx, userAccount, signer, provider);
  } catch (e) {
    console.log(e);
  }
};
...
const disableWhitelist = async () => {
  ...
  try {
    ...
    const disableWhitelistTx = getDisableWhitelistTransaction(
      smartContract,
      5000000
    );

    await commonTxOperations(disableWhitelistTx, userAccount, signer, provider);
  } catch (e) {
    console.log(e);
  }
};
...
export const nftMinter = async (subcommand?: string) => {
  ...
  switch (subcommand) {
    ...
    case COMMANDS.getTokensMintedPerAddress:
      ...
      break;
    case COMMANDS.enableWhitelist:
      enableWhitelist();
      break;
    case COMMANDS.disableWhitelist:
      disableWhitelist();
      break;
    case COMMANDS.addToWhiteList:
      addToWhiteList();
      break;
    case COMMANDS.getWhitelist:
      commonScQuery({
        functionName: 'getWhitelist',
        resultLabel: 'Whitelist addresses: ',
        resultType: 'string',
      });
      break;
  }
