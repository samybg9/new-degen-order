...
//+++  
export const getAddToWhitelistTransaction = (
  contract: SmartContract,
  gasLimit: number,
  address: string
) =>
{
  return contract.call({
    func: new ContractFunction('addToWhitelist'),
    args: [new AddressValue(new Address(address.trim()))],
    gasLimit: new GasLimit(gasLimit)
  })
}

//+++
export const getEnableWhitelistTransaction = (
  contract: SmartContract,
  gasLimit: number
) => {
  return contract.call({
    func: new ContractFunction('enableWhitelist'),
    gasLimit: new GasLimit(gasLimit),
  });
};

//+++
export const getDisableWhitelistTransaction = (
  contract: SmartContract,
  gasLimit: number
) => {
  return contract.call({
    func: new ContractFunction('disableWhitelist'),
    gasLimit: new GasLimit(gasLimit),
  });
};
